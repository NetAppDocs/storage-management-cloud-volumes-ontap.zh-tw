...
NOTE: NetApp 已限制購買、延期及續約 BYOL 授權，並限制特定情境（例如私有模式部署）中的可用度。如需詳細資訊，請參閱 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html#restricted-availability-of-byol-licensing-for-cloud-volumes-ontap["Cloud Volumes ONTAP的 BYOL 授權可用性受限"^]。
...
* *市場訂閱/年度合約*：指您從雲端供應商的市場購買的年度合約。私有模式部署不支援市場訂閱。
...
* 如果您擁有 BYOL 授權並透過市集簽訂了年度合約，您的使用量和超額使用量將根據您設定的計費偏好收取費用。若要了解您的收費方式，請參閱 link:concept-licensing-charging.html#billing-preferences-and-overages["計費偏好和超額費用"]。
...
如需定價的詳細資訊，請前往 link:https://www.netapp.com/pricing/["NetApp 定價網站"^]。
...
* * NetApp優先授權*：在此模式下，您的使用量首先會根據您的許可證包（自帶許可證）的容量進行計費。如果您超出授權容量，超出部分將根據您的年度市場訂閱或市場按需小時費率（PAYGO）收取費用。如果您的 BYOL 授權到期，您必須透過雲端市場過渡到基於容量的授權模式。更多資訊請參閱 link:task-convert-node-capacity.html["將Cloud Volumes ONTAP基於節點的許可證轉換為基於容量的許可證"]。
* *僅限市場訂閱用戶*：在此模式下，您的使用費用將首先計入您的年度市場訂閱費用。任何額外使用均按市場按需小時費率（PAYGO）收費。任何未使用的許可證容量在計費時均不予考慮。


有關帳單偏好設定的更多信息，請參閱 https://docs.netapp.com/us-en/console-licenses-subscriptions/concept-billing-preferences.html["了解授權和訂閱的計費方式"^]。
...
如需 Essentials 授權收費的相關資訊，請參閱 <<基本套裝>>。
