...
https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-limits-gcp.html["Google Cloud 中 Cloud Volumes ONTAP 的儲存限制"^]
...
機器類型:: 查看支援的機器類型link:reference-configs-gcp.html["Cloud Volumes ONTAP發行說明"^]，然後審查 Google 提供的每種支援機器類型的詳細資訊。根據您的工作負載需求，選擇與機器類型對應的 vCPU 數量和記憶體容量。請注意，每個 CPU 核心都會提升連網效能。
+
--
請參閱以下內容以了解更多詳細資訊：
...
--
磁碟類型:: 為Cloud Volumes ONTAP建立磁碟區時，您需要選擇Cloud Volumes ONTAP用於磁碟的底層雲端儲存。磁碟類型可以是以下任一種：
+
--
...
--
磁碟大小:: 部署Cloud Volumes ONTAP系統時，您需要選擇初始磁碟大小。之後，您可以讓NetApp Console為您管理系統的容量，但如果您想自行建立聚合，請注意以下事項：
+
--
* 聚合中的所有磁碟必須具有相同的大小。
* 確定所需的空間，同時考慮性能。
...
* link:reference-configs-gcp.html#google-cloud-single-node["查看 Google Cloud 中Cloud Volumes ONTAP系統資料的預設磁碟"]。
...
TIP: Console 代理程式也需要系統磁碟。 https://docs.netapp.com/us-en/console-setup-admin/reference-connector-default-config.html["查看控制台代理預設配置的詳細信息"^]。
