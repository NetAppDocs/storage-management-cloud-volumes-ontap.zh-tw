...
.關於此任務
過渡工具可在 NetApp 支援網站中找到，它會產生以下文件：
...
** 請確保您已完成網路配置並為私有模式部署建立了工作池。請參閱link:reference-networking-gcp.html#infrastructure-manager-configuration-in-private-mode-or-protected-environments["私有模式部署的 Infrastructure Manager 組態"]。
...
[%collapsible]
====
指定  `--worker-pool` 參數以在私有模式部署中執行該工具。有關工作池配置，請參閱 link:reference-networking-gcp.html#infrastructure-manager-configuration-in-private-mode-or-protected-environments["私有模式部署的 Infrastructure Manager 組態"]。
