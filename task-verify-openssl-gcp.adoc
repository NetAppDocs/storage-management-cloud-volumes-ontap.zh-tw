...
summary: 使用 OpenSSL 驗證 Google Cloud 下載的 disk.raw 檔案是否與摘要文件內容一致。 
---
= 使用 OpenSSL 驗證Cloud Volumes ONTAP的 Google Cloud 映像 disk.raw 文件
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
您可以透過以下方式驗證 Google Cloud 下載的 disk.raw 檔案與摘要檔案內容 https://mysupport.netapp.com/site/products/all/details/cloud-volumes-ontap/downloads-tab["國家安全局"^]使用 OpenSSL。
...
----
====
. 將下載的 disk.raw 檔案、簽名和憑證放在一個目錄中。
. 使用 OpenSSL 從憑證中提取公鑰。
. 使用提取的公鑰解密簽名並驗證下載的 disk.raw 檔案的內容。
+
.點擊顯示
[%collapsible]
====
