...
[role="lead"]
Cloud Volumes ONTAP 需要 Google Cloud 服務帳戶，用於兩個目的。第一個是當您啟用 link:concept-data-tiering.html["資料分層"] 將冷資料分層至 Google Cloud 中的低成本物件儲存時。第二個是當您啟用 https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html["NetApp Backup and Recovery"^] 將 Volume 備份至低成本物件儲存時。
...
.步驟
. 在 Google Cloud Console 中 https://console.cloud.google.com/iam-admin/serviceaccounts["前往服務帳戶頁面"^]。
. 選擇您的項目。
. 點擊*建立服務帳戶*並提供所需資訊。
+
.. *服務帳戶詳細資料*：輸入名稱和描述。
...
+
image:screenshot_gcp_service_account_grant_access.gif["Google Cloud Console 的螢幕截圖，您可以在建立服務帳號時授予使用者存取權限。"]
