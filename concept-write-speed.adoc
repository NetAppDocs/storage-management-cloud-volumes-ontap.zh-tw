...
link:reference-configs-aws.html["了解有關Cloud Volumes ONTAP支援的 Amazon EC2 執行個體的更多信息"]。
...
如果您使用 HA 配對，Cloud Volumes ONTAP 從 9.8 版本開始支援多種 VM 類型的高速寫入。請前往 link:reference-configs-azure.html["Cloud Volumes ONTAP發行說明"] 檢視支援高速寫入的 VM 類型。
...
如果您使用 HA 配對，Cloud Volumes ONTAP 從 9.13.0 版本開始支援多種 VM 類型的高速寫入。請前往 link:reference-configs-gcp.html#supported-configurations-by-license["Cloud Volumes ONTAP發行說明"^] 檢視支援高速寫入的 VM 類型。

link:reference-configs-gcp.html["詳細了解Cloud Volumes ONTAP支援的 Google Cloud 機器類型"^]。
