...
如需設定 Proxy 伺服器的相關資訊，請參閱 https://docs.netapp.com/us-en/console-setup-admin/task-configuring-proxy.html["配置控制台代理以使用代理伺服器"^]。
...
控制台會建立 Azure 安全性群組，其中包含 Cloud Volumes ONTAP 成功運作所需的傳入和傳出規則 https://docs.netapp.com/us-en/console-setup-admin/reference-ports-azure.html["查看控制台代理程式的安全性群組規則"^]。
...
我們不建議修改預先定義的安全性群組或使用自訂安全性群組。但是，如果您必須這樣做，請附註，部署程序要求 Cloud Volumes ONTAP 系統在其自身子網路內擁有完全存取。部署完成後，如果您決定修改網路安全性群組，請確保叢集連接埠和 HA 網路連接埠保持開啟，因為 HA 連網依賴兩個節點之間不受限制的通訊。此外，節點的 IP 位址在部署期間會自動定義。這確保了 Cloud Volumes ONTAP 叢集內部的無縫通訊（節點之間任意通訊）。
...
* https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html["查看控制台代理程式的網路要求"^]
