...
---
= 在 Azure 的 Cloud Volumes ONTAP 中隔離 SnapMirror 流量
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
----
.. 儲存 privateIPAddress。此 IP 位址 <new_added_nic_primary_addr> 用於在 <<step-second,廣播網域，以及新 NIC 的叢集間 LIF>> 中建立叢集間 LIF。
...
== [[step-second]]步驟 2：為新 NIC 建立新的 IPspace、廣播網域和叢集間 LIF
