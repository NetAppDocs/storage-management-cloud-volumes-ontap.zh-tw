...
+
** 您應該有一個 https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["與您的系統關聯的控制台代理"^]。
** https://docs.netapp.com/us-en/console-setup-admin/concept-connectors.html["您應該準備好讓控制台代理程式始終處於運行狀態"^]。
** 與 Console 代理程式關聯的服務帳戶 https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html["應該具有所需的權限"^]。
...
. 如果系統提示， https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["建立控制台代理"^]。
...
| 服務帳戶名稱 | 如果您打算將 link:concept-data-tiering.html["資料分層"] 或 https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html["NetApp Backup and Recovery"^] 與 Cloud Volumes ONTAP 搭配使用，則需要啟用 *服務帳戶* 並選擇具有預先定義儲存管理員角色的服務帳戶。link:task-creating-gcp-service-account.html["了解如何建立服務帳號"^]。 
...
NOTE: 這是您為 Console 設定的服務帳戶，link:https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["如本頁所述"^]。

按一下「新增訂閱」將選定的憑證與訂閱關聯。

若要建立隨用隨付的 Cloud Volumes ONTAP 系統，您需要從 Google Cloud 市場中選擇一個與 Cloud Volumes ONTAP 訂閱相關的 Google Cloud 專案。請參閱 https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["將市場訂閱與 Google Cloud 憑證關聯"^]。
...
. *預先配置套件*：選擇其中一個套件以快速部署 Cloud Volumes ONTAP 系統，或按一下*建立我自己的組態*。預先配置套件會因所選的 Cloud Volumes ONTAP 版本而異。例如，對於 Cloud Volumes ONTAP 9.18.1 及更新版本，NetApp Console 會顯示包含 C3 VM 的套件，包括 Hyperdisk Balanced 磁碟。您可以根據工作負載需求修改組態，例如 IOPS 和處理量參數。
+
...
+
如需選擇磁碟類型和大小的協助，請參閱link:task-planning-your-config-gcp.html#size-your-system-in-google-cloud["在 Google Cloud 中調整系統大小"^]。

. *快閃記憶體快取、寫入速度和 WORM*：
+
.. 如有需要，請啟用 *Flash Cache* 或選擇 *Normal* 或 *High* 寫入速度。
+
深入瞭解 link:reference-configs-gcp.html#flash-cache-support["快閃記憶體"] 和 link:concept-write-speed.html["寫入速度"]。

+

NOTE: 透過 *High* 寫入速度選項，可提供高寫入速度和 8,896 位元組的較高最大傳輸單元（MTU）。此外，8,896 的較高 MTU 需要為部署選擇 VPC-1、VPC-2 和 VPC-3。如需 VPC-1、VPC-2 和 VPC-3 的詳細資訊，請參閱  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html#requirements-for-the-console-agent["VPC-1、VPC-2 和 VPC-3 的規則"^]。
...
** 如需新增 Google Cloud 帳戶的協助，請參閱  https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["使用 9.6 設定和新增 Google Cloud 帳戶以進行資料分層"^]。
...
| NTP 伺服器 | 選擇「使用 Active Directory 網域」以使用 Active Directory DNS 設定 NTP 伺服器。如果需要使用其他位址設定 NTP 伺服器，則應使用 API。如需相關資訊，請參閱 https://docs.netapp.com/us-en/console-automation/index.html["NetApp Console自動化文檔"^]以取得詳細資料。請注意，只能在建立 CIFS 伺服器時設定 NTP 伺服器。建立 CIFS 伺服器後，將無法設定 NTP 伺服器。 
|===
...
. *系統詳情*：選擇一個專案，指定叢集名稱，可選擇服務帳戶，可選擇新增標籤，然後指定憑證。
+
下表描述了您可能需要指導的欄位：
...
| Google Cloud 專案和 Marketplace 訂閱  a| 
選擇要用於此系統的 Google 專案和訂閱。這是您為 Console 設定的服務帳號。如需相關資訊，請參閱 link:https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["主控台設定文件"^]。Console 使用專案建立 Cloud Volumes ONTAP 系統，並使用訂閱確定計費和使用情況。如果在下拉清單中看不到任何其他專案，則表示您尚未將該服務帳號與其他專案建立關聯。請前往 Google Cloud Console，開啟 IAM 服務，然後選擇專案。將用於 Console 的角色所對應的服務帳號新增至該專案。您需要對每個專案重複此步驟。

按一下「新增訂閱」將選定的憑證與訂閱關聯。

若要建立隨用隨付的 Cloud Volumes ONTAP 系統，您需要從 Google Cloud Marketplace 中選擇一個與 Cloud Volumes ONTAP 訂閱關聯的 Google Cloud 專案。請參閱 Console 設定文件： https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["將市場訂閱與 Google Cloud 憑證關聯"^]。
...
| 服務帳戶名稱 | 如果您打算使用 link:concept-data-tiering.html["NetApp Cloud Tiering"] 或  https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html["備份和復原"^] 服務，則需要啟用 *服務帳戶* 交換器，然後選擇具有預先定義 Storage Admin 角色的服務帳戶。 


| 標籤 | 標籤是您的 Google Cloud 資源的元資料。控制台將標籤新增至Cloud Volumes ONTAP系統以及與該系統關聯的 Google Cloud 資源。建立系統時，您可以從使用者介面新增多達四個標籤，然後可以在建立系統後新增更多標籤。請注意，在建立系統時，API 不會將您限制為四個標籤。有關標籤的信息，請參閱 https://cloud.google.com/compute/docs/labeling-resources["Google Cloud 文件：標記資源"^]。 


| ONTAP 使用者名稱和密碼 | 這些是Cloud Volumes ONTAP叢集管理員帳戶的憑證。您可以使用這些憑證透過ONTAP System Manager 或ONTAP CLI 連線到Cloud Volumes ONTAP 。保留預設的_admin_使用者名稱或將其變更為自訂使用者名稱。 
|===
...
. *部署拓樸*：HA 設定可以選擇多個可用區 (AZ)（建議）或單一 AZ。
+
link:concept-ha-google-cloud.html["了解有關 HA 部署模型的更多信息"^] 。

. *網路配置和連線*：選擇相關的區域和可用區。為 HA 配置選擇四個不同的 VPC，每個 VPC 中選擇一個子網路，然後選擇防火牆原則。
+

NOTE: Cloud Volumes ONTAP 9.19.1 及更高版本支援link:concept-ha-google-cloud.html["用於單一可用區高可用性配置的共用磁碟"]，使中介器執行個體成為選用項目。在此配置中，您無需輸入中介器區域、對應的 VPC 和子網路即可進行資料複製。
...
+
如果您選擇其中一個套件，只需指定一個磁碟區，然後審核並核准組態即可。預先配置套件會根據所選的 Cloud Volumes ONTAP 版本和組態類型而有所不同。例如，如果您選擇 Cloud Volumes ONTAP 9.19.1 或更新版本的單一 AZ 部署，則可以看到包含 C3 系列機器和 Hyperdisk Balanced 磁碟的預先配置套件。對於舊版和多個 AZ，您仍然可以選擇 n2-standard 機器和 Persistent Disks。您可以根據工作負載需求修改組態，例如 IOPS 和輸送量參數。
...
NOTE: 透過 n2-standard-16、n2-standard-32、n2-standard-48 和 n2-standard-64 執行個體類型的 *High* 寫入速度選項，可提供高寫入速度和 8,896 位元組的較高最大傳輸單元（MTU）。此外，較高的 MTU 8,896 需要在部署時選擇 VPC-1、VPC-2 和 VPC-3。高寫入速度和 MTU 8,896 取決於功能，無法在已設定的執行個體中單獨停用。如需 VPC-1、VPC-2 和 VPC-3 的詳細資訊，請參閱  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html#requirements-for-the-connector["VPC-1、VPC-2 和 VPC-3 的規則"^]。
...
| NTP 伺服器 | 選擇 *Use Active Directory Domain* 以使用 Active Directory DNS 設定 NTP 伺服器。如果需要使用其他位址設定 NTP 伺服器，則應使用 API。詳情請參閱 https://docs.netapp.com/us-en/console-automation/index.html["NetApp Console自動化文檔"^]。請注意，只能在建立 CIFS 伺服器時設定 NTP 伺服器。建立 CIFS 伺服器後將無法設定 NTP 伺服器。 
|===
. *使用情況設定檔、磁碟類型和分層策略*：選擇是否要啟用儲存效率功能並變更磁碟區分層策略（如果需要）。
+
更多信息，請參閱link:task-planning-your-config-gcp.html#choose-a-volume-usage-profile["選擇卷使用情況設定檔"^]，link:concept-data-tiering.html["資料分層概述"^] ， 和 https://kb.netapp.com/Cloud/Cloud_Volumes_ONTAP/What_Inline_Storage_Efficiency_features_are_supported_with_CVO#["KB：CVO 支援哪些內嵌儲存效率功能？"^]
