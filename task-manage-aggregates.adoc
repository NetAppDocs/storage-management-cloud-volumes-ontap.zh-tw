...
. 從系統詳細資料中，按一下「*聚合*」標籤。
. 針對所需的彙總，請按一下 image:icon-action.png["一個顯示三個水平點的圖示，用於執行其他動作"] 圖示以進行管理動作。
+
image:screenshot_aggr_menu_options.png["聚合選單選項的螢幕截圖。"]

. 透過 image:icon-action.png["一個顯示三個水平點的圖示，用於執行其他動作"] 選單中的可用選項管理您的彙總。
...
. 在 image:icon-action.png["一個顯示三個水平點的圖示，用於執行其他動作"] 選單下，點選 *增加容量*。
