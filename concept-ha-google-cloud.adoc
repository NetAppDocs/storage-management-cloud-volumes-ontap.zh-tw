...
傳統上，Google Cloud 中的 Cloud Volumes ONTAP 高可用性配置採用非共享模型，其中使用中介實例來管理容錯移轉操作或監控節點健康狀況。這需要在 Google Cloud 中建立額外的 VM，並為每個節點配置兩組磁碟。
...
存取實例:: 對於 Debian 系統，預設雲端使用者是 `admin`。當透過 Google Cloud Console 或 gcloud 命令列請求 SSH 存取時，Google Cloud 會為 `admin`使用者建立並新增憑證。您可以指定 `sudo`來取得 root 權限。
...
Cloud Volumes ONTAP 9.19.1 及更高版本支援單一 AZ 的共享 HA 配置。在共享 HA 架構中，兩個節點可以共享一個磁碟，因此聚合的建立方式有所不同。不再為兩個 VM 建立兩個磁碟，而是建立一個磁碟並將其附加至 VM。共享 HA 不需要磁碟鏡像。
...
多個可用區（建議）:: 跨三個區域部署 HA 配置可確保當一個區域內發生故障時資料仍然可用。請注意，與使用單一區域相比，寫入效能略低，但差異很小。
單一可用區:: 當部署在單一可用區域 (AZ) 中時，Cloud Volumes ONTAP HA 組態會採用分散放置原則。此原則可確保 HA 組態免受區域內單點故障的影響，而無需使用個別的區域來實現故障隔離。
+
--
