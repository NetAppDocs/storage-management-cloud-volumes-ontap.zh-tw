...
* Cloud Volumes ONTAP 提供高達 500 GiB 已佈建容量的免費授權，不含 NetApp 支援，且無到期日。超過 500 GiB 的容量需要有效的授權權利，並可能產生費用。若要存取完整的 Cloud Volumes ONTAP 體驗和進階管理功能，請將您的 Cloud Volumes ONTAP 系統連線至主控台。透過主控台進行探索和管理需要與您的主控台帳戶關聯的有效 Azure Marketplace 訂閱。
...
.開始之前
* 如果您打算在 Console 中探索和管理已部署的系統，請確保 Console 帳戶已關聯至有效的 Azure Marketplace 訂閱。將系統探索至 Console 需要有效的訂閱。
