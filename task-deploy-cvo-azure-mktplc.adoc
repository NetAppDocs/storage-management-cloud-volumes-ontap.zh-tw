...
summary:  
---
= 從 Azure Marketplace 部署 Cloud Volumes ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
您可以使用 Azure Marketplace 直接部署來快速輕鬆地部署 Cloud Volumes ONTAP。只需在 Azure Marketplace 中點擊幾下，即可快速部署 Cloud Volumes ONTAP，並在您的環境中探索其核心特性和功能。

有關該產品的更多信息，請參閱link:concept-azure-mktplace-direct.html["了解NetApp Console和市場中的Cloud Volumes ONTAP產品"]。

.關於此任務
使用 Azure Marketplace 直接部署所部署的 Cloud Volumes ONTAP 系統具有下列屬性。請注意，透過 Azure Marketplace 部署的獨立式執行個體的功能，會在 NetApp Console 中被探索時發生變更。

* 最新的Cloud Volumes ONTAP版本（9.16.1 或更高版本）。
* Cloud Volumes ONTAP 提供高達 500 GiB 已佈建容量的免費授權，不含 NetApp 支援，且無到期日。超過 500 GiB 的容量需要有效的授權權利，並可能產生費用。若要存取完整的 Cloud Volumes ONTAP 體驗和進階管理功能，請將您的 Cloud Volumes ONTAP 系統連線至 NetApp Console。透過 NetApp Console 進行探索和管理，需要與您的 NetApp Console 帳戶關聯的有效 Azure Marketplace 訂閱。
...
.開始之前
* 如果您打算在 NetApp Console 中探索和管理已部署的系統，請確保 NetApp Console 帳戶已關聯至有效的 Azure Marketplace 訂閱。將系統探索至 NetApp Console 需要有效的訂閱。
...
.步驟
. 從 Azure Marketplace 站台搜尋 NetApp 產品。
...
透過 Azure Marketplace 直接部署的 Cloud Volumes ONTAP 系統不包含來自 NetApp 的支援。如果在部署過程中出現任何問題，您可以自行進行疑難排解並解決。

.步驟
. 在 Azure Marketplace 站台上，前往 *Boot diagnostics > Serial log*。
. 下載並調查串行日誌。
. 請參閱產品文件和知識庫 (KB) 文章以進行故障排除。
+
** https://learn.microsoft.com/en-us/partner-center/["Azure Marketplace 文件"]
...
您可以探索使用 Azure Marketplace 直接部署所部署的 Cloud Volumes ONTAP 系統，並在 Console 的 *Systems* 頁面上管理這些系統。Console 代理程式會探索這些系統、新增這些系統並套用必要的授權，並為這些系統解鎖 Console 的完整功能。單一 AZ 中使用 PSSD v2 Managed Disks 的原始 HA 組態將予以保留，且系統會註冊至與原始部署相同的 Azure 訂閱和資源群組。

.關於此任務
在探索使用 Azure Marketplace 直接部署所部署的 Cloud Volumes ONTAP 系統時，Console 代理程式會執行下列工作：
...
.開始之前
請確保 Azure Marketplace 中的部署已完成。Console 代理程式只有在部署完成且系統可供探索後，才能探索這些系統。

.步驟
在控制台中，您可以按照標準程序來發現現有系統。請參閱link:task-adding-systems.html["將現有的Cloud Volumes ONTAP系統新增至控制台"] 。


CAUTION: 在探索期間，您可能會看到故障訊息，但您可以忽略這些訊息，直到探索程序完成。在探索期間，請勿修改 Azure Marketplace 入口網站中系統產生的 Cloud Volumes ONTAP 組態，尤其是系統標記。對這些組態所做的任何變更都可能導致意外的系統行為。
...
* https://docs.netapp.com/us-en/console-licenses-subscriptions/task-manage-subscriptions.html["新增新的市場訂閱"^]
