...
在 Google Cloud 中啟用資料分層後，Cloud Volumes ONTAP 會使用持久磁碟作為熱資料的效能層，並使用 Google Cloud Storage 儲存桶作為非使用中資料的容量層。您可以在建立系統時選擇儲存層，並可隨時變更。有關變更儲存類別的資訊，請參閱 link:task-tiering.html["將非活動資料分層到低成本物件存儲"]。
...
--
儲存類別:: Google Cloud 使用 https://cloud.google.com/storage/docs/storage-classes["儲存類別"^]來管理成本和資料可用性。我們建議您根據預期的資料存取頻率選擇儲存類別，因為存取資料會產生較高的成本。分層資料的預設儲存類別為 _Standard Storage_。除非手動變更，所有非使用中的資料都將維持預設儲存類別。若要變更非使用中資料的儲存類別，請參閱 https://docs.cloud.google.com/storage/docs/managing-lifecycles["Google Cloud 文件：管理物件生命週期"^]。
+
--
資料分層的儲存類別是系統範圍的 - 而不是每個磁碟區的。
...
.複製
複製磁碟區時，您可以選擇是否將資料分層到物件儲存。如果這樣做，控制台會將*備份*政策套用至資料保護磁碟區。從Cloud Volumes ONTAP 9.6 開始，*全部*分層策略取代了備份策略。刪除複製關係時，目標磁碟區將保留複製期間生效的分層策略。
