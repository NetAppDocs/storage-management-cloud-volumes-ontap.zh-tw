...
.步驟
. 選擇 System Manager 檢視以透過 ONTAP System Manager 調整 Volume 大小。請參閱 link:task-administer-advanced-view.html#get-started-with-system-manager["開始使用系統管理員"]。
. 從左側導覽選單中，選擇“儲存”>“磁碟區”。
. 從磁碟區清單中，確定應調整大小的磁碟區。
...
TIP: 當彙總的已使用容量達到 90% 時，主控台會顯示這些「需要採取行動」訊息。如果啟用資料分層，當彙總的已使用容量達到 80% 時，就會顯示這些訊息。預設情況下，會預留 10% 的可用空間以進行資料分層。link:task-tiering.html#change-the-free-space-ratio-for-data-tiering["了解有關數據分層的可用空間比率的更多信息"^]。
...
.. 在彙總圖塊上，按一下 image:icon-action.png["一個顯示三個水平點的圖示，用於執行其他動作"] 圖示，然後按一下 *檢視彙總詳細資料*。
...
+
如需詳細資訊，請參閱link:https://docs.netapp.com/us-en/data-services-replication/task-replicating-data.html["在系統之間複製數據"^]。

. 前往「複製狀態」頁面，然後中斷SnapMirror關係，將複製的磁碟區從資料保護磁碟區轉換為讀取/寫入磁碟區。
+
如需詳細資訊，請參閱link:https://docs.netapp.com/us-en/data-services-replication/task-replicating-data.html#managing-data-replication-schedules-and-relationships["管理資料複製計劃和關係"^]。
...
.. 在所需的彙總圖塊上，按一下 image:icon-action.png["一個顯示三個水平點的圖示，用於執行其他動作"] 圖示，然後按一下 *檢視彙總詳細資料* 以檢視可用容量（已配置大小減去已使用彙總容量）。
+
...
. 如果需要，將磁碟新增至現有聚合：
+
.. 選擇彙總，然後按一下 image:icon-action.png["一個顯示三個水平點的圖示，用於執行其他動作"] 圖示 > *新增磁碟*。
.. 選擇要新增的磁碟數量，然後按一下「*新增*」。
