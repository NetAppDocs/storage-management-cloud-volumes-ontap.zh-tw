...
---
= 使用 AWS 中的進階選項部署 Cloud Volumes ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* 已啟動並正在執行的控制台代理程式。
+
** 您應該有一個 https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-aws.html["與您的系統關聯的控制台代理"^]。
...
. 如果出現提示， https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-aws.html["建立控制台代理"^]。
...
| 編輯憑證 | 選擇與您要部署此系統的帳戶相關聯的 AWS 憑證。您也可以關聯要與此 Cloud Volumes ONTAP 系統搭配使用的 AWS 市場訂閱。按一下 *新增訂閱* 將所選憑證與新的 AWS 市場訂閱關聯。訂閱可以是年度合約，或是以每小時費率支付 Cloud Volumes ONTAP 費用。https://docs.netapp.com/us-en/console-setup-admin/task-adding-aws-accounts.html["了解如何在NetApp Console新增其他 AWS 憑證"^]。 
|===
+
...
. *位置與連線*：輸入您在 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config.html#collect-networking-information["AWS 工作表"^]中記錄的網路資訊。
+
下表描述了您可能需要指導的欄位：
...
| 編輯憑證 | 選擇要與此 Cloud Volumes ONTAP 系統搭配使用的 AWS 憑證和市場訂閱。按一下 *新增訂閱* 將所選憑證與新的 AWS 市場訂閱建立關聯。訂閱可以是年度合約，也可以按小時費率支付 Cloud Volumes ONTAP 費用。如果您直接向 NetApp 購買授權（自帶授權（BYOL）），則不需要 AWS 訂閱。NetApp 已限制 BYOL 授權的購買、延展和續約。如需詳細資訊，請參閱 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html#restricted-availability-of-byol-licensing-for-cloud-volumes-ontap["Cloud Volumes ONTAP的 BYOL 授權可用性受限"^]。https://docs.netapp.com/us-en/console-setup-admin/task-adding-aws-accounts.html["了解如何在控制台中新增其他 AWS 憑證"^]。 
|===
+
...
+
** https://docs.netapp.com/us-en/data-services-data-classification/concept-cloud-compliance.html["了解有關NetApp Data Classification的更多信息"^]
** https://docs.netapp.com/us-en/data-services-backup-recovery/task-backup-to-s3.html["了解有關備份和恢復的更多信息"^]
+
...
+
IP 位址必須位於該地區所有 VPC 的 CIDR 區塊之外。如需其他詳細資料，請參閱 link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-aws.html#requirements-for-ha-pairs-in-multiple-azs["多個可用區中Cloud Volumes ONTAP HA 的 AWS 網路需求"^]。
...
. *使用情況設定檔、磁碟類型和分層原則*：選擇是否要啟用儲存效率功能，並在需要時編輯磁碟區分層策略。
+
如需更多資訊，請參閱link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config.html#choose-a-volume-usage-profile["選擇卷使用情況設定檔"^]和link:concept-data-tiering.html["資料分層概述"^]。
