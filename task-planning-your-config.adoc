...
NOTE: 如果您的 Cloud Volumes ONTAP 系統位於 AWS Local Zone，則不支援將資料分層儲存到 Amazon Simple Storage Service（Amazon S3），因為在 Local Zone 之外存取 Amazon S3 儲存貯體會造成更高的延遲，並影響 Cloud Volumes ONTAP 活動。
...
link:reference-configs-aws.html#aws["查看 AWS 中Cloud Volumes ONTAP系統資料的預設磁碟"]。
