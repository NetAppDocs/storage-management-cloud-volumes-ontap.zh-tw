...
summary: 您可以在 AWS Secret Cloud 和 Top Secret Cloud 環境中部署 Cloud Volumes ONTAP。AWS Secret Cloud 和 Top Secret Cloud 區域是專為美國情報機構提供的封閉區域。本頁的說明僅適用於 AWS Secret Cloud 和 Top Secret Cloud 區域的使用者。 
---
...
+
若要深入瞭解私有模式的運作方式，請參閱link:https://docs.netapp.com/us-en/console-setup-admin/concept-modes.html#private-mode["控制台中的私有部署模式"^]。
...
. 設定到 Amazon Simple Storage Service（Amazon S3）服務的 VPC 端點。
+
