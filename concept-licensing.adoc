...
[role="lead"]
Cloud Volumes ONTAP 提供多種授權模式。每種模式都能讓您選擇滿足自身需求的消費模式。

您可以選擇透過雲端市場或透過 NetApp （自帶授權 (BYOL)）購買或續約 Licenses and subscriptions。市場提供年度訂閱和隨用隨付 (PAYGO) 選項。如需相關資訊，請參閱 link:concept-license-management.html["了解有關Cloud Volumes ONTAP基於容量的許可證的更多信息"]。

BYOL 授權可向 NetApp 購買，並可用於任何雲端供應商。有關 BYOL 授權可用度的詳細資訊，請參閱 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html#restricted-availability-of-byol-licensing-for-cloud-volumes-ontap["Cloud Volumes ONTAP的 BYOL 授權可用性受限"^]。
...
新客戶可以選擇按容量付費的年度市場訂閱或 Keystone 訂閱。少數部署（例如私有模式部署）也採用上一代基於節點的 BYOL 授權模式。

基於容量的許可:: 按配置容量支付NetApp帳戶中的多個Cloud Volumes ONTAP系統的費用。包括購買附加雲端資料服務的能力。有關基於容量的許可證的消費模式或購買選項的更多信息，請參閱：link:concept-licensing-charging.html["了解有關基於容量的許可證的更多信息"] 。
...
* link:reference-configs-azure.html["Azure 中支援的配置"]
...
* link:reference-configs-gcp.html["Google Cloud 中支援的配置"^]
...
NOTE: 備份和復原需要隨用隨付（PAYGO）訂閱，但使用此服務不會產生任何費用。如需設定備份和復原授權的詳細資訊，請參閱 https://docs.netapp.com/us-en/data-services-backup-recovery/task-licensing-cloud-backup.html["設定備份和恢復許可"^]。
...
https://docs.netapp.com/us-en/console-setup-admin/concept-connectors.html#impact-on-cloud-volumes-ontap["了解控制台代理"^]。


NOTE: NetApp 已限制 BYOL 授權的購買、延期和續約。如需詳細資訊，請參閱 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html#restricted-availability-of-byol-licensing-for-cloud-volumes-ontap["Cloud Volumes ONTAPP的 BYOL 授權可用性受限"^]。
...
NOTE: 在私有環境中以私有模式部署 Cloud Volumes ONTAP。
...
|===
.例外情況
NetApp 認識到某些情況需要特殊考量，例如在 AWS 中部署 Cloud Volumes ONTAP 的中國區域，基於節點的授權的 EOA 和 EOS 可能不適用於以下情況：
...
.相關連結
* link:concept-licensing-charging.html["了解有關基於容量的許可證的更多信息"]
* link:task-manage-capacity-licenses.html["管理Cloud Volumes ONTAP基於容量的許可"]
* link:task-manage-node-licenses.html["管理節點型授權"]
* link:task-convert-node-capacity.html["將基於節點的許可證轉換為基於容量的許可證"]
* link:task-manage-keystone.html["管理Keystone訂閱"]
