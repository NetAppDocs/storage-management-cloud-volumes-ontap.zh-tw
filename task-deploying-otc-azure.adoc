...
---
= 使用 Azure 中的進階選項部署 Cloud Volumes ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* 已啟動並正在執行的控制台代理程式。
+
** 您應該有一個 https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html["與您的系統關聯的控制台代理"^]。
...
. 如果出現提示， https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html["建立控制台代理"^]。
...
| 資源組標籤 | 標籤是 Azure 資源的元資料。當您在此欄位中輸入標籤時，控制台會將它們新增至與Cloud Volumes ONTAP系統關聯的資源群組。建立系統時，您可以從使用者介面新增多達四個標籤，然後可以在建立系統後新增更多標籤。請注意，在建立系統時，API 不會將您限制為四個標籤。有關標籤的信息，請參閱 https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/tag-resources["Microsoft Azure 文件：使用標籤來組織您的 Azure 資源"^]。 
...
| 編輯憑證 | 您可以為此 Cloud Volumes ONTAP 系統選擇不同的 Azure 憑證和不同的 Azure 訂閱。您需要將 Azure Marketplace 訂閱與所選的 Azure 訂閱建立關聯，才能部署隨用隨付的 Cloud Volumes ONTAP 系統 https://docs.netapp.com/us-en/console-setup-admin/task-adding-azure-accounts.html["了解如何新增憑證"^]。 
|===
. *服務*：啟用或停用您想要或不想與Cloud Volumes ONTAP一起使用的單一服務。
+
** https://docs.netapp.com/us-en/data-services-data-classification/concept-cloud-compliance.html["了解有關NetApp Data Classification的更多信息"^]
...
TIP: 如果您使用的 Azure 帳戶具有 https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-azure.html["所需權限"^]，則在部署故障或刪除的情況下，控制台會將 Cloud Volumes ONTAP 資源從資源群組移除。
...
| 使用現有的 | 如果您選擇現有的安全性群組，則它必須符合 Cloud Volumes ONTAP 的要求。link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-azure.html#security-group-rules["查看預設安全群組"^]。 
|===
...
NOTE: 如果所選版本有更新的候選版本、正式版或修補程式版本可用，則在建立工作環境時，Console 會將系統更新至該版本。例如，如果您選擇 Cloud Volumes ONTAP 9.16.1 P3，而 9.16.1 P4 可用，則會進行更新。更新不會從一個版本升級到另一個版本，例如，不會從 9.15 升級到 9.16。
...
+
如需選擇磁碟類型和大小的協助，請參閱link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#size-your-system-in-azure["在 Azure 中調整系統大小"^]。
...
+
此選項僅適用於某些虛擬機器類型。若要了解支援哪些虛擬機器類型，請參閱 link:reference-configs-azure.html#ha-pairs["HA 對許可證支援的配置"]。
...
+
如需更多資訊，請參閱link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#choose-a-volume-usage-profile["了解卷使用情況"^]和link:concept-data-tiering.html["資料分層概述"^]。
...
** 磁碟大小適用於初始聚合中的所有磁碟以及使用簡單配置選項時控制台建立的任何其他聚合。您可以使用進階分配選項建立使用不同磁碟大小的聚合。
+
如需協助選擇磁碟大小，請參閱link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#size-your-system-in-azure["在 Azure 中調整系統大小"^]。
...
+
link:reference-configs-azure.html#ha-pairs["詳細了解 Azure 中支援的配置"^]。
