...
磁碟:: 客戶資料儲存在 Premium Storage page blobs 中。每個節點都可以存取其他節點的儲存設備。link:reference-configs-azure.html#azure-ha-pair["引導、根和核心數據"] 也需要額外的儲存設備。
儲存帳戶::
+
--
* 託管磁碟需要一個儲存帳戶。
...
NOTE: 從 Cloud Volumes ONTAP 9.15.0P1 開始，Azure 頁面 Blob 不再支援新的 HA 配對部署。如果您目前在現有的 HA 配對部署中使用 Azure 頁面 Blob，則可以移轉至 Edsv4 系列 VM 和 Edsv5 系列 VM 中較新的 VM 執行個體類型。link:reference-configs-azure.html#ha-pairs["詳細了解 Azure 中支援的配置"]。
...
NOTE: 如果您為符合上述條件的環境選擇進階 SSD 管理磁碟，控制台將自動部署進階 SSD v2 管理磁碟。您無法切換到進階 SSD v1 管理磁碟。

磁碟:: 客戶資料儲存在本地冗餘儲存設備（LRS）代管的磁碟上。每個節點都可以存取其他節點的儲存設備。link:reference-configs-azure.html#azure-ha-pair["啟動、根、合作夥伴根、核心和NVRAM數據"] 也需要額外的儲存設備。
...
磁碟:: 客戶資料儲存在區域冗餘儲存設備（ZRS）代管的磁碟上。每個節點都可以存取其他節點的儲存設備。此外，還需要額外的儲存設備供 link:reference-configs-azure.html#azure-ha-pair["啟動、根、合作夥伴根和核心數據"] 使用。
儲存帳戶:: 儲存帳戶用於基於託管磁碟的部署，以處理診斷日誌和分層到 Blob 儲存。
...
NOTE: Cloud Volumes ONTAP 使用負載平衡器以主備模式進行偵測。在 Azure 中，只有活動節點會回應探測。備用節點可能會顯示為非活動狀態，並帶有 X 標記。這是預期行為，無需擔心。

.相關連結
link:task-azure-high-availability-mode.html["在 Azure 中為Cloud Volumes ONTAP啟用高可用性模式"^]
