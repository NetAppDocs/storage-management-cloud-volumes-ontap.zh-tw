...
Console 代理程式會聯絡多個端點以進行日常作業。如需所使用端點的相關資訊，請參閱 https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html#step-3-set-up-networking["查看從控制台代理聯繫的端點"^]與 https://docs.netapp.com/us-en/console-setup-admin/reference-networking-saas-console.html["準備好使用控制台的網絡"^]。
...
在 Cloud Volumes ONTAP 部署期間，AWS 中介代理欄位接受伺服器位址。如果您輸入的是完全限定網域名稱（FQDN）而不是 IP 位址，則除非 AWS VPC 配置了自訂 DNS 伺服器，否則 FQDN 將無法解析。這是因為 AWS 提供的預設 DNS 僅解析公用 DNS 記錄和內部 AWS 主機，無法解析外部 FQDN，例如客戶代理伺服器。如果這種情況適用於您，您有兩個選擇：

* 直接在 mediator proxy 欄位中新增 proxy 伺服器的 IP 位址，而不是 FQDN。
* 在您的 VPC 中設定 DHCP 選項集，其中包含您的自訂 DNS 伺服器位址。這樣，當您在部署期間將 FQDN 輸入到 proxy 伺服器欄位時，AWS 就可以解析該 FQDN。如需詳細資訊，請參閱 https://docs.aws.amazon.com/vpc/latest/userguide/DHCPOptionSet.html["AWS 文件"^]。
...
TIP: 正在尋找有關 Console 代理程式的資訊？ https://docs.netapp.com/us-en/console-setup-admin/reference-ports-aws.html["查看控制台代理程式的安全性群組規則"^]
...
如果您想要將 EBS 用作效能層，將 Amazon S3 用作容量層，則必須確保 Cloud Volumes ONTAP 與 S3 建立連線。提供此連線的最佳方法是建立指向 S3 服務的 VPC 端點。有關說明，請參閱 https://docs.aws.amazon.com/AmazonVPC/latest/UserGuide/vpce-gateway.html#create-gateway-endpoint["AWS 文件：建立網關終端節點"^]。
...
* https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-aws.html["查看控制台代理程式的網路要求"^]
